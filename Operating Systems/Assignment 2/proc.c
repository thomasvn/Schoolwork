 ...
 */
  ...
  for (q=0; q < NR_SCHED_QUEUES; q++) {	
	...
	/* If you previously jumped somewhere else randomly, return to previous it */	
	if (randFlag == 1) {
		q = temp + 1;
		randFlag = 0;
	}

	/* rp: running process pointer
	 * rdy_head: array of queues 
	 * Picks the head of the priority queue 
	 */
	
	/* This process has high priority and should not be touched */
	if (q > 6) {
		/* Returns q from 0 - 8 */
		q = rand() % 9;

		/* Add 7 to move to range 7 - 15 */
		q += 7;

		/* Let yourself know that you have randomly chosen the next one */
		temp = q;
		randFlag = 1;
	}
	...
	/* 
 	 ...
 	 */
	if(!(rp = rdy_head[q])) {
		/* Configure, Record, and Display Kernel Trace Events */
		TRACE(VF_PICKPROC, printf("queue %d empty\n", q););
		continue;
	}

#if DEBUG_RACE
	/* Chooses a random process in the queue to run */

 ...
 *
 *
 * c) Add code to the relevant class(es) to count the no. of times, the CRITICAL_LEVEL is reached. Show the count.
 *
 * 
 */
...
class LevelMonitor implements Observer {
	...
	protected WaterTank tank; // this is the Observable object that should be observed
	private int critical_level_counter = 0;
	...
	public void update(Observable o, Object t) {
		...
		if (volume <= this.CRITICAL_LEVEL) {
			...
			System.out.println("Number of times levels were critical: " + critical_level_counter);
